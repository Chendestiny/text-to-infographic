...
## 视觉调用的延迟实测（同一张封面图，7 个尺寸）

| 尺寸 | 1440px | 1080px | 900px | 720px | 540px | 420px | 300px |
|---|---|---|---|---|---|---|---|
| 延迟 | 46.9s | 49.2s | 55.8s | 31.7s | 60.2s | 73.1s | 66.4s |

**延迟与图片尺寸无关**（像素只占 1.9% 的 300px 反而比占 44% 的 1440px 慢）——是服务商波动。
成本看**调用次数**与**输出长度**：一次问 4 张的拼版调用 >600s 未返回。
所以：一次只问一个问题、只要一行答案；默认不看图；要看只看 1~2 张 720px 缩略图。
