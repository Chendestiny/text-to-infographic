...
**第 6 步 · 逐张自检 → 交付**
两道门只保证"没溢出、没吞字"，不保证好看。交付前**逐张看图**，专找门看不见的四类：
**断词**（`schema` 折成 `sche` / `ma`）、**孤字**（末行只剩 1~2 字）、**压字/出框**、
**数量词对不上**（标题写"两件事"却列了 4 条）。页多就并行开子任务复核，只看它们的结论。
