...
| 交付前看图复核 | `python scripts/review_sheet.py <出图目录>` | 720px 缩略图 + **2×2 拼版图**（一次看 4 张） |
...
**第 6 步 · 看图复核 → 交付**
两道门只保证"没溢出、没吞字"，不保证好看。但**别拿原图逐张硬看** —— 实测那样 12 次视觉分析
吃掉一整轮 **77% 的时间**（单次 20~236 秒，模型还在写整段"图像整体描述"）。先跑：

```bash
python scripts/review_sheet.py <出图目录>     # 720px 缩略图 + 2×2 拼版（每格带 card-XX 角标）
```

把 `review/sheet-*.jpg` 交给看图模型，**一次看 4 张**，并且只问四类毛病（这句能挡住"写作文"）：

> 只回答四类毛病：断词（`schema` 折成 `sche`/`ma`）、孤字（末行只剩 1~2 字）、压字/出框、
> 数量词对不上（标题写"两件事"却列了 4 条）。不要写图像描述。每张一行：`card-XX: 无问题` 或现象。

要细看某一页，再单独给 `review/thumb/card-XX.jpg`。页多就并行分片（每个 subagent 几片），你只看结论。

- **没有 vision provider**：改一次配置指到一个多模态模型是值得的，但**别反复折腾**——
  改不动就在交付报告里写"审美未校验（harness 无 vision provider）"，不要卡在这一步
- 省掉的是时间，不是检查项：四类毛病一个都不能省
...
- [ ] **复核过拼版图**（断词 / 孤字 / 压字 / 数量词），发现的都已改文案重渲；没 vision provider 就写明"审美未校验"
