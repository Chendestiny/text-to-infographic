...
把 plan.py 打印的 `"plan": {...}` **原样复制进规格的 `meta`** —— 门禁按它判合规：

- 页数 = 建议值（或落在区间内）→ 通过
- **最多上浮 1 张**，且必须把理由填进 `meta.plan.note`；理由要具体到
  "骨架里哪一页承载不了原文的哪个重点"，不是"我觉得 9 张好看"
- **超过上沿 +1 就是硬违规**：默认动作是**合并相邻页**，不是写理由。
  真要更多张，就重新规划成系列（每篇 6~8 张）—— 写理由不是无限上浮的通行证

```bash
python scripts/plan.py <文章.md> --check <spec.json>    # 按同一套上限对账
```
